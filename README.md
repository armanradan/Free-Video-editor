...
The Windows native correctness harness uses installed FFmpeg/FFprobe 8.0.1 on `PATH`. It offers a direct-FFmpeg software route and a comparison route that decodes to RGBA with FFmpeg, applies the **same `media-gpu` WGSL resize**, then reads back RGBA to FFmpeg's software encoder. Both use `media-core` resize/profile policy; H.264/AAC 8-bit output is available on both routes, while HEVC Main 10/AAC output is direct-only. The CLI defaults to direct FFmpeg for resize-only conversion; select `--route wgpu` explicitly for the comparison route. Keep wgpu for custom effects shared with the web app or a GPU effect chain that proves worthwhile in complete conversion benchmarks. The current wgpu route explicitly counts CPU→GPU and GPU→CPU frame bytes; it is not zero-copy hardware codec interoperability. Both routes accept fixed-size, 8-bit opaque YUV 4:2:0 BT.709 limited-range SDR (or unspecified color tags) MP4 input with one video and at most one audio track; the Main 10 direct route also accepts 10-bit 4:2:0 SDR. HDR, wide color, and full range are rejected. Both routes retain specified BT.709 tags on tested HEVC input. Direct FFmpeg preserves verified VFR/non-negative A/V offsets and bakes the tested crop/SAR/orientation fixture into square-pixel pixels; shared wgpu still requires square-pixel, unrotated, zero-origin CFR because its raw-frame bridge does not carry the requisite geometry or per-frame timestamps. Ctrl-C requests cooperative cancellation and reaps FFmpeg children; `--cancel-after-ms N` is a deterministic cancellation-test option. Native Dioxus/Blitz UI, HDR conversion, and direct codec-surface sharing are not implemented yet.
