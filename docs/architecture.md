...
Status: design researched 2026-09-15; M3.1 through M3.6 pass their acceptance gates in the available Firefox/Chromium environments as of 2026-09-25. Configurable output, bounded streaming, named sizes, source metadata, capability-gated HEVC, controlled recovery, storage fallback/cleanup, explicit limit checks, and long-run stress are implemented. See [interop-report.md](interop-report.md) for milestone-specific evidence and limitations. Safari, non-Windows systems, other GPUs/drivers, spontaneous process/driver loss, and HEVC output on a supporting browser remain untested compatibility coverage rather than claims of support.
...
HEVC extension (2026-09-24): `Mp4H265Aac` is a fourth concrete profile using Mediabunny's HEVC MP4 path and the existing AAC, scheduling, GPU, muxing, cancellation, and verification machinery. The MP4 input adapter accepts both AVC and HEVC after exact decoder probing. H.265 output is shown only when the exact HEVC/AAC encoder configuration passes; it is never substituted for H.264. The deterministic `m36-h265-aac.mp4` fixture proves HEVC input through the full Chromium decode → wgpu resize → VP8 encode → re-decode path. The tested Chromium environment rejected HEVC encoding and Firefox rejected HEVC decoding/encoding, so output execution remains unverified while its unavailable-state behavior is verified.
...
Implementation status (2026-09-20): acceptance passed for the tested environments. Every stage now reports live/peak/total/discarded counts, while the underlying Mediabunny decoder, WebCodecs encoder, and mux policies retain their separate bounds. The GPU path uses a device-generation-tagged four-slot input-texture ring per configured size and reports allocations, reuse, copies, captures, CPU bridge/submission time, overlapping completion latency, slot-reuse wait, and final-drain wait. Each slot retains its decoded frame and optional compatibility bitmap until that slot's completion callback; reuse retires the previous submission first, and completion/cancellation drains the ring. This removed the earlier per-frame completion barrier while keeping at most four GPU lifetimes. Firefox's deterministic 60-frame conversions improved from roughly 6.2 seconds to 1.2–1.8 seconds without changing codecs, confirming that the old serialization was the dominant short-job cost. After the first real copy establishes that bitmap compatibility ingress is required, subsequent bitmaps are prepared in bounded groups of four and consumed in timestamp order; cancellation closes every prepared or pending bitmap. Firefox exposed only two concurrent bitmap tasks and still spent roughly 0.9–1.0 seconds of cumulative task latency, so this produced only a modest additional wall-time improvement and did not eliminate the remaining browser bridge cost. Production timing now stops after encode/mux finalization; full output re-decode/seek/audio verification is an explicit `?verify=full` diagnostic whose time is reported separately and remains enabled in interoperability tests. A UI selector carries an exact `no-preference` or `prefer-hardware` request through worker and fallback paths; exact decoder and encoder probes choose the requested setting only when the complete profile passes, otherwise the baseline fallback and reason are visible. On the tested browsers, `prefer-hardware` encoding was unsupported for every enabled profile, so no automatic hardware preference or hardware-codec benefit is claimed. The selected wgpu backend exposes no released direct VideoFrame import, and reliable codec-attributed CPU/power data was not observable; both remain explicitly unmeasured rather than inferred. M3.5 was subsequently completed as recorded below.
...
Implementation status (2026-09-21): acceptance passed for the tested Firefox/Chromium environments. `media-core` now represents coded/visible/square-pixel geometry and the ordered rotation/flip transform. At the browser boundary, decoded `VideoFrame` external-image import normalizes the visible crop, pixel aspect ratio, and supported SDR color conversion into a square-pixel sRGB processing texture; the shared WGSL processor then applies clockwise rotation followed by horizontal flip while resizing. Output is always square-pixel, unrotated, and unflipped because geometry is baked into pixels. The accepted input policy is opaque BT.709/sRGB SDR; BT.2020/PQ or other HDR/wide-color combinations fail before conversion. Mid-stream coded size, visible rectangle, square-pixel size, orientation, or color changes fail the active job and drain owned resources rather than silently reconfigure. Integer timestamps and durations are preserved through WebCodecs and verified packet-by-packet after muxing, allowing only the selected container's timebase quantization; the final packet may derive its omitted duration from separately verified track coverage. The deterministic geometry/color and VFR/non-zero-origin fixtures passed every profile enabled in Edge 153 and Firefox 156, including decoded marker colors, midpoint seek, audio checks, and A/V offsets. The tested H.264 decoders normalized SPS clean-aperture cropping into a full decoded `visibleRect`; non-zero decoder-exposed crop offsets remain implemented but were not produced by these browser/codec combinations.
...
#### M3.6 — configurable output, streaming, compatibility matrix, and recovery

Implementation status (updated 2026-09-25): the configurable-output and bounded-streaming slices passed their tested Firefox/Chromium matrix. `media-core::ResizeSpec` now provides original, validated percentage, named HD 720p, Full HD 1080p, 2K-width, QHD 1440p, 4K UHD, and exact-bound output policy. Named modes are aspect-preserving maximum bounds, not crop/stretch promises; they retain the no-upscale rule and show the actual codec-safe result. Exact output preserves display aspect ratio by default; the explicit unlocked mode stretches to the requested dimensions. Resolution happens after crop, pixel-aspect normalization, and orientation, disallows implicit upscaling, and floors codec dimensions to visible even values. The UI shows the selected input's display/coded resolution, H.264/H.265 codec string, duration, average frame rate, frame count, audio codec/channels/rate, and file size. It re-probes every output profile at the exact resolved size before conversion. Full browser encode/re-decode checks passed original, 75%, 50%, 25%, exact 500×500 locked, and exact odd 501×301 unlocked output in every enabled profile; cancellation after a size change and invalid-upscale rejection returned owned resources to zero. Input uses an 8 MiB bounded BlobSource cache. Compressed output uses a backpressure-aware Mediabunny `StreamTarget` with 4 MiB chunks over origin-private file storage, then exposes the disk-backed `File` through the existing download URL; the current file remains while that URL is usable and is removed on replacement or page exit. Both browsers converted a valid logical 257.3 MiB sparse MP4 that previously crossed the fixed limit. If origin-private streaming is unavailable, the status names the memory fallback and its retained 256 MiB input cap; forced mode, automatic API-unavailable fallback, and oversized-input rejection passed Chromium compatibility tests.

The recovery slice now has deterministic, test-only codec and WebGPU device-loss injection. Codec failure is injected after five GPU-processed frames; the device-loss case destroys the active wgpu device after four completed frames. Both failures stop the job, drain submitted GPU work, close/discard owned frame, bitmap, sample, codec, muxer, and partial-output resources, and leave the page ready for an explicit retry. A lost session is invalidated while its retained canvas permits a new session and device generation in both worker/OffscreenCanvas and main-thread contexts. Same-page retries passed full output re-decode in Firefox and Chromium: codec retries retained generation 1, while device-loss retries recreated generation 2. This validates the controlled application recovery boundary, not spontaneous driver reset, process loss, an indefinitely hung codec callback, or automatic checkpoint/resume.

Final M3.6 acceptance (2026-09-25): the real-browser resize matrix includes the 50% mode, producing 18 Chromium and 12 Firefox profile/size conversions with full re-decode. A page-exit cleanup command removes the current execution context's retained OPFS entry without deleting entries potentially owned by another tab; synthetic pagehide and real same-origin navigation returned both browsers to their recorded pre-test baselines. Chromium also passed the normal automatic memory-fallback branch with `getDirectory` unavailable in the tested main-thread context, without the `?output=memory` switch. The selected Chromium adapter reported a 16,384-pixel 2D texture limit; the app checks input/output dimensions against the actual wgpu device limit before resource creation, and direct exact VP8 probes accepted 8,192² but rejected 16,384² and above. A 70,439,352-byte, 147.3-second local input produced 3,530 frames and a 32,138,436-byte output through 17 bounded writes (maximum exactly 4 MiB), passed complete timeline/audio re-decode, ended all application ownership counters at zero, and kept the worker-hosted page responsive. This closes the M3.6 acceptance gate for available environments; the compatibility limitations above remain explicit.

Profile-selector correction (2026-09-25): output availability is defined by the selected input decoder, audio track, frame rate, and resolved dimensions. The profile selector and Convert action therefore remain disabled until that exact file probe succeeds. A new file or size choice invalidates the previous result while probing. Initial WebM options are no longer presented as selectable while MP4 options await input-specific checks. Chromium and Firefox browser harnesses verified the disabled initial state and the expected post-input H.264 MP4 availability in each browser.

Initial resize selection (2026-09-25): the UI now starts at Original size. The 50% mode remains available and is chosen explicitly by historical half-size regression harnesses; the resize matrix verifies Original as the initial selection and fully re-decodes every enabled profile at that size.

- Replace the fixed `ResizePreset::Half` UI policy with a platform-neutral `ResizeSpec` owned by `media-core`. Initially expose original size, percentage presets (75%, 50%, 25%), and exact width/height. Preserve display aspect ratio by default, offer an explicit aspect-ratio lock, and never silently stretch, crop, or upscale.
- Resolve resize dimensions only after visible crop, pixel-aspect-ratio normalization, and rotation. Define deterministic fit behavior when both exact dimensions are supplied, round codec-constrained dimensions to valid even values with the adjusted result visible before conversion, and reject zero, overflowed, GPU-limit, or encoder-limit sizes clearly.
- Carry the resolved size through profile capability probing, GPU configuration/resource reuse, canvas/export, filename/status metadata, and full output verification. Re-probe every enabled output profile for the exact chosen dimensions instead of assuming a preset-supported encoder configuration.
- Add focused `media-core` tests for aspect preservation, fit boxes, odd-dimension adjustment, rotated/non-square-pixel input, no-upscale policy, and overflow. Add real-browser coverage for original, percentage, and exact-size output across the enabled profile matrix, including cancellation/restart after changing size.
...
**Acceptance gate:** every enabled profile produces verified original-size, percentage, and exact-size outputs with the displayed resolved dimensions, preserved aspect ratio, correct orientation, and valid codec geometry; invalid requests fail before conversion. An input larger than the former limit completes without retaining the entire input or output in application memory; the compatibility report lists exact tested browser/profile combinations; injected failure and available device-loss tests return owned resources to zero and permit restart.

#### M3.7 — conditional FFmpeg WASM backend spike

Historical M3.7 status (superseded by M3.8's raw-frame ring below): an explicitly selected `?backend=ffmpeg-wasm` MP4/H.264/AAC compatibility spike shares the M3 browser job, WebCodecs decode, metadata checks, and wgpu processing with the default WebCodecs path. A browser-local `OutputStorageKind` policy gives both codec output adapters the same OPFS output contract; WebCodecs retains its bounded `StreamTarget` and memory compatibility fallback, whereas FFmpeg requires OPFS. M3.7 received one wgpu-produced `VideoFrame.copyTo` RGBA readback at a time, spooled it to OPFS, read the original `File` through Emscripten WORKERFS, and read/wrote the raw spool and final MP4 through worker-side synchronous OPFS file devices. Its prior 64 MiB whole-source and 128 MiB in-memory raw caps were replaced by a 2 GiB raw-disk-spool policy with a browser quota preflight; source size was no longer capped by this adapter. It accepted integer constant rates 1–120 fps plus 24000/1001, 30000/1001, and 60000/1001, and still required exactly one audio track and exact per-frame timing. The UI marks WebM/HEVC choices unavailable in FFmpeg mode, and WebCodecs remains the default. Both available browsers passed the 60-frame fixture and a valid 257.3 MiB sparse MP4 source through FFmpeg, with full re-decode, independent ffprobe, cancellation, and same-page retry. This was a disk-backed compatibility route, not a no-readback fast path or proof of bounded FFmpeg internal/WASM memory. Core assets are large and GPL-licensed; full browser-process peak resident memory, broad geometry/VFR cases, distribution compliance, and production resource suitability remain unresolved. See `docs/interop-report.md` for measured results.

Unavailable FFmpeg profiles keep selected input metadata and show the exact rejected reason beside the disabled selector, including a route back to WebCodecs. In M3.7, the 2 GiB raw-RGBA spool bound was an application disk policy, not a source-size cap or a guarantee that all accepted clips fit in FFmpeg's WASM address space. The source and output were streamed at the filesystem boundary, but raw frames were spooled before encoding rather than encoded concurrently with GPU processing. M3.8 removes that raw spool; a much larger clip may still exhaust compressed-output OPFS quota or FFmpeg internal memory, neither of which is silently retried in memory.
...
#### M3.8 — bounded FFmpeg frame stream

Implementation status (2026-09-25): the FFmpeg WASM adapter now uses the four-slot, 4 MiB `SharedArrayBuffer` ring below. The generated FFmpeg worker mounts it as a non-seekable synchronous Emscripten input device, starts `exec` before frame production finishes, and streams the final compressed MP4 to OPFS. No raw-frame OPFS file or 2 GiB raw-spool policy remains. The source still uses WORKERFS; WebCodecs and shared wgpu processing are unchanged. FFmpeg mode requires a cross-origin-isolated page and non-blocking `Atomics.waitAsync`; `dx serve --web --cross-origin-policy` supplies COOP/COEP for development, and production hosting must supply equivalent headers. Both available Firefox and Chromium environments passed full re-decode on a synthetic 2.21 GB logical-RGBA / 2,400-frame case, including H.264/AAC and independent FFprobe; details and outstanding limits are in `docs/interop-report.md`. The FFmpeg path still performs one explicit RGBA GPU readback and a copy to the WASM heap per frame, and its internal memory is not bounded by the ring.

**Goal and scope.** Remove the M3.7 *raw intermediate frame* OPFS spool and its 2 GiB policy limit. Do not change the default WebCodecs backend, the shared wgpu processor, codec/profile selection, or the compressed-input/output storage policy. The selected source `File` may still be read through WORKERFS and the final compressed MP4 may still stream to OPFS; “no raw disk persistence” does not mean keeping a whole output movie in memory. FFmpeg WASM still requires one explicit `VideoFrame.copyTo` GPU→CPU RGBA readback per processed frame, followed by a copy into its WASM heap. This milestone cannot claim a no-readback or zero-copy FFmpeg path. It is an optional follow-up to M3.7, not a prerequisite for M4.

**Backend boundary.** Keep one browser job/profile/timestamp policy and one wgpu processor. The concrete WebCodecs output adapter continues to accept processed `VideoFrame`s and stream encoded packets through Mediabunny. The concrete FFmpeg output adapter accepts the same processed frames but publishes RGBA bytes through a bounded `EncoderIngress` transport. A browser-local enum can describe the actually exercised ingress (`DirectVideoFrame` for WebCodecs, `SharedRgbaRing` for FFmpeg); the existing `OutputStorageKind` continues to describe *compressed-output* storage independently. Do not move `SharedArrayBuffer`, DOM, FFmpeg FS, or OPFS types into `media-core`, and do not add a generic Rust trait merely for this transport. The FFmpeg adapter owns the ring and the small worker-side FS device shim. The Dioxus UI only displays capability, progress, and errors.

```text
WebCodecs: decoded frame → shared wgpu processor → VideoFrame → WebCodecs encoder → compressed output stream
FFmpeg:   decoded frame → shared wgpu processor → VideoFrame.copyTo(one reusable RGBA frame buffer)
         → bounded shared-memory byte ring → FFmpeg worker synchronous FS read → libx264/AAC → compressed OPFS output
```

**Why a shared ring.** The pinned FFmpeg WASM worker calls FFmpeg's synchronous `exec`, and its Emscripten FS device `read` callback is synchronous. A normal `postMessage` queue or async `ReadableStream` cannot supply the next bytes while that worker is blocked inside `exec`: its message handler cannot run until the call returns. A `SharedArrayBuffer` ring lets the *consumer* block with `Atomics.wait` inside the dedicated FFmpeg worker while the producer, in the browser processing worker or main-thread compatibility path, continues asynchronously. The producer must never call blocking `Atomics.wait` on the page thread; wait for released slots using `Atomics.waitAsync` only after a capability check, or worker-to-producer release notifications plus an atomic recheck to avoid missed wakeups. This is a dedicated-worker-only blocking consumer, not a promise that FFmpeg's public API natively accepts browser streams. [Emscripten device callbacks](https://emscripten.org/docs/api_reference/Filesystem-API.html#devices), [Atomics.wait restrictions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Atomics/wait).

**Protocol and boundedness.** Use a fixed byte budget (four 1 MiB slots), with per-slot atomic `FREE`/`READY` state and published byte length, plus atomic `RUNNING`/`EOF`/`ABORT` control; the surrounding browser job owns stale-callback/generation handling. Copy only into a free slot; publish length and then `READY` atomically. The FS `read` callback consumes bytes in strict slot order, supports arbitrary request sizes across slot/frame boundaries, releases each fully consumed slot, and returns zero *only* after EOF and all published bytes have drained. Temporary ring emptiness is not EOF: wait for enough data to satisfy a read, except for the final short read at true EOF. It must not seek backwards; the pinned FFmpeg rawvideo demuxer accepts this non-seekable device in the tested browsers. Check multiplication and counters for overflow, including recordings that pass 2 GiB of total bytes. Retain at most one full RGBA frame buffer on the producer side, chunk it into the ring, then reuse it only after all of its bytes have been copied into bounded ring slots; the bridge's explicit JS storage bound is one frame plus the ring, not the number of frames. This does **not** bound FFmpeg's opaque WASM heap, browser copies, or the decoded/GPU pools. Keep the current exact constant-frame-rate and timestamp checks until FFmpeg is given a validated way to consume per-frame timing; a byte stream alone carries no VFR timestamps.

**Startup, drain, and failure order.** Load the core, mount the read-only source `File`, writable compressed-output OPFS device, and raw-video ring device before starting FFmpeg `exec`. Start `exec` without awaiting it, then allow the existing bounded video pump to publish frames while the audio pump runs concurrently. Only after the last processed video frame should the producer publish EOF; wait for the ring to drain and for FFmpeg to finish audio encode/mux/flush before exposing the output `File`. Validate `published bytes = processed frames × width × height × 4`, consumed bytes, FFmpeg exit status, encoded frame count, and existing full output timeline/audio checks. A producer/GPU/decoder failure, FFmpeg exception/early exit, device loss, or user cancellation must atomically publish ABORT, notify all waiters, invalidate the job generation, stop both pumps, terminate the FFmpeg worker if it does not exit promptly, and discard the partial compressed output. Never let one side await a full/empty ring after its peer has failed. Preserve the existing frame/GPU lease lifetime rules and same-page retry behavior.

**Availability and rollout.** Shared memory requires the page and workers to be `crossOriginIsolated`, normally through `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp` or `credentialless`; check actual browser support, `SharedArrayBuffer`, and relevant policy permissions at runtime. Configure and test those headers for the Dioxus development server *and* intended production hosting; audit third-party scripts/assets and embedded-browser behavior under COEP. Do not assume a secure localhost alone grants shared memory. If the ring is unavailable, report the exact FFmpeg-streaming capability failure and offer the independently probed WebCodecs profiles. Any retention of the M3.7 OPFS raw-spool path must be an explicitly labeled compatibility choice with its own bound, never a silent fallback or a claim that M3.8 is active. Do not make cross-origin isolation a prerequisite for the default WebCodecs path. [Cross-origin isolation requirements](https://developer.mozilla.org/en-US/docs/Web/API/Window/crossOriginIsolated), [worker shared memory](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers).

**Implementation sequence and acceptance.** First prove the pinned FFmpeg core can synchronously consume a bounded, non-seekable ring device with a small deterministic rawvideo fixture; pin and guard the generated worker modification against dependency-layout changes. Then integrate it behind the FFmpeg adapter's `start`/`add`/`finalize` lifecycle without changing the shared processing math. Instrument ring occupancy/high-water mark, producer wait, consumer wait, total published/consumed bytes, readback count, WASM linear-memory high-water mark, and completion/cancel cleanup. In real Firefox and Chromium sessions, require full browser re-decode and independent FFprobe/FFmpeg checks for video count/timestamps, audio, and seekability; test consumer-starved and producer-blocked cancellation, encoder failure, device loss, and same-page retry. Include a reproducible, small-compressed fixture whose *logical* RGBA output exceeds the former 2 GiB spool cap, plus representative source resolutions; verify no raw OPFS entry is created and bounded bridge occupancy throughout. Record browser/OS/GPU and isolation headers in `docs/interop-report.md`. A passing tiny fixture alone, successful compilation, or a simulated ring without FFmpeg `exec` is not sufficient. If isolation, non-seekable reads, correctness, or memory behavior fails, keep M3.7 as the clearly limited FFmpeg compatibility route and document the failed boundary; do not weaken the WebCodecs path or silently substitute disk persistence.

Core M3 browser robustness is complete when M3.1 through M3.6 pass. M3.7 and M3.8 are optional compatibility extensions and are not required to begin M4. Partial completion must be reported by sub-milestone number and must not be presented as completion of all browser robustness work.

### M4 — native correctness using shared processing

M4 status (2026-09-25): complete for the tested Windows headless direct-FFmpeg subset; other systems and advanced media cases remain outside this acceptance claim. Implementation: `media-native` provides a Windows-tested FFmpeg 8.0.1 executable backend with two routes: direct FFmpeg decode/`scale`/encode as a software baseline, and FFmpeg raw-RGBA decode → shared `media-gpu::ResizePipeline` WGSL pass → readback → FFmpeg software encode. Both use `media-core` output-size/profile policy, bounded per-frame pipes, decoded-frame geometry/timestamp preflight, and MP4/H.264/AAC output checks; direct FFmpeg supports verified VFR/non-negative origins while shared wgpu still requires zero-origin CFR. The native CLI lists wgpu adapter descriptors and accepts/persists an exact descriptor key in an explicit preference file; it re-resolves that key on each run and reports fallback if unavailable. A shared cancellation token stops either route; scoped child guards kill/reap FFmpeg on error or cancellation, and an unpublished partial output is removed after children exit. A native watcher terminates both FFmpeg processes on cancellation even while the conversion thread blocks in a decoder read or encoder write; focused Windows tests cover both pipe directions. The CLI handles Ctrl-C; the GPU map wait checks cancellation between bounded polls. This is **not** zero-copy codec/GPU interoperability: the shared-wgpu route counts one CPU→GPU upload and one GPU→CPU readback per frame, with a completion wait per frame. The tested NVIDIA GPU route is slower than direct FFmpeg on the small fixture; this is a measured baseline, not a universal GPU-performance conclusion. See `docs/interop-report.md`.

A headless `NativeSession` now serializes jobs, cancels and waits for an active job before changing adapter preference, and advances a device-generation ID. Each GPU job creates fresh resources, so nothing is reused across generations. A two-physical-GPU same-process switch passed on Windows. There is no Dioxus Native/Blitz UI or CLI live-switch command. This does not prove recovery from a hung GPU driver or spontaneous device loss; broader fixture/metadata coverage remains pending outside the supported subset. Hardware codec-surface interop is deferred to M5.

Native GPU failure handling now registers wgpu's device-lost callback on each job device. The callback records the reason without calling back into wgpu, and signals the FFmpeg child watcher so a blocked decoder read or encoder write can release. Frame submission/readback boundaries check the recorded loss, fail the job, reap children, and discard the partial output. A controlled `Device::destroy()` injection after one processed frame passed same-process cleanup and fresh-device retry on Windows/NVIDIA; the `Destroyed` callback is not evidence for spontaneous `Unknown` driver loss or a driver stall that never signals. Every retry constructs a new device and frame resources rather than reusing the lost device. The test hook is private to the native harness and not exposed by the CLI. [wgpu device-lost callback and destroy API](https://docs.rs/wgpu/30.0.1/wgpu/struct.Device.html)

A reproducible release-mode native harness now generates seeded, textured 1080p/60-frame and 4K/30-frame H.264/AAC sources under ignored `tmp`, then compares direct FFmpeg and shared-wgpu 50% resize outputs, full decodability, audio, counts, durations, and decoded-video PSNR. This covers one synthetic high-frequency pattern on Windows/NVIDIA; it does not replace representative camera footage, longer high-resolution runs, other hardware, or device-loss tests. See the measured results in `docs/interop-report.md`.

The direct FFmpeg route now accepts the checked-in VFR/non-zero-origin fixture. It uses `-copyts -start_at_zero`, passthrough frame sync, and an explicit 1/90000 video encoder time base; native preflight scans actual decoded frame PTS and geometry, and post-encode validation compares every normalized video timestamp plus A/V offset and stream duration before publishing output. The shared-wgpu route retains its explicit zero-origin CFR rejection: raw RGBA pipes currently convey no per-frame timestamps to the encoder, so permitting VFR there would silently change timing. The route distinction is reported as capability, not an automatic fallback. This is a tested subset, not arbitrary edit-list/negative-origin or multi-track support.

The direct route also accepts the checked-in SDR crop/non-square-SAR/rotation-plus-flip fixture. FFprobe stream dimensions represent its display aperture differently from decoded-frame dimensions, so direct preflight checks that decoded dimensions stay constant rather than requiring equality to the stream header. The codec boundary resolves SAR and reported rotation into `media-core::FrameGeometry` for display-size-based resize policy; FFmpeg's autorotation applies the display matrix, including the fixture's reflection, before scaling. The output must have square pixels, no display matrix, matching specified color tags, and a verified frame timeline before publication. Independent decoded-corner sampling confirms the fixture's orientation and marker colors. The shared-wgpu raw-RGBA path continues to reject this input: it has no verified aperture/SAR/matrix bridge yet. This is one supported SDR fixture, not universal matrix decomposition or colorimetric proof.

Native color policy is now explicit for both routes: accept only 8-bit opaque YUV 4:2:0 (`yuv420p`/`nv12`) with BT.709 limited-range SDR or unspecified tags, and reject tagged HDR/wide-color/full-range inputs before conversion. The direct FFmpeg route retains source tags through its source filter chain; the shared-wgpu raw-RGBA encoder must explicitly set both FFmpeg and libx264 VUI options for each specified tag. Post-encode checks require those tags to match before the partial output is published. This was verified with the tagged HEVC/AAC and HDR fixtures, but does not prove pixel-level colorimetric fidelity or support for untagged material whose actual primaries differ from the assumed SDR baseline.

The direct route additionally offers `mp4-h265-main10-aac` for 10-bit BT.709 limited-range SDR. This distinct `media-core::OutputProfileId` carries a 10-bit output-depth policy without exposing an unverified browser profile. Native preflight accepts `yuv420p10le`/`p010le` only for this direct profile; `probe_source` can report their metadata independently of route. FFmpeg scales and encodes as `yuv420p10le` with libx265 and an `hvc1` MP4 sample entry, with source color tags carried into x265 VUI. Post-encode verification requires HEVC Main 10, `yuv420p10le`, `hvc1`, square pixels, no residual display matrix, expected frame/timeline/audio metadata, and matching specified color tags before publication. The checked-in 10-bit gradient/AAC fixture exercises this path. Eight-bit input may be encoded into Main 10 but does not acquire lost source precision; the existing 8-bit H.264 profile refuses 10-bit input rather than silently downconverting it. The shared-wgpu path rejects the Main 10 profile before opening output because its raw RGBA8 bridge cannot preserve 10-bit samples. HDR/BT.2020/PQ remains a separate unsupported color pipeline; 10-bit alone is not HDR support.

**Native processing-route decision (2026-09-25).** Direct FFmpeg is the native CLI default for resize-only conversion and for effect graphs that FFmpeg can implement with acceptable output parity. The headless CLI accepts `--route wgpu` for the comparison path; no native UI or effect-aware automatic route selector has shipped. Keep the shared-wgpu route as an explicit alternative for custom effects whose implementation/behavior must be shared with the web app, or for a GPU effect chain that demonstrably benefits from staying on one processing device. Do not send every native job through wgpu merely to reuse the shader, and do not assume that more effects automatically make it faster: native FFmpeg also has software and hardware filters. For a wgpu job, select the route and adapter before creating codec/GPU resources, keep successive effects on the same device, and cross the CPU↔GPU boundary at most once in each direction per frame with the current software codecs. Record those transfers and compare complete decode→effects→encode time, visual/timing/audio correctness, and resource use against a direct FFmpeg graph on representative high-detail inputs and in release builds before changing the default. A future same-adapter codec-surface bridge may change the result, but is an M5 interop proof, not a present capability or reason to silently route ordinary jobs through readback. Keep shared `media-core` policy/capability reporting for either route; unsupported effect combinations require an explicit route/capability result, not a silent change of output semantics. Route choice is per job, never mid-frame or as an unannounced fallback after a job starts. The browser WebCodecs→wgpu path remains a separate, already validated browser backend.
