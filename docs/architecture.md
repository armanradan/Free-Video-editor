...
Implementation status (2026-09-25, headless slice): `media-native` provides a Windows-tested FFmpeg 8.0.1 executable backend with two explicit routes: direct FFmpeg decode/`scale`/encode as a software baseline, and FFmpeg raw-RGBA decode → shared `media-gpu::ResizePipeline` WGSL pass → readback → FFmpeg software encode. Both use `media-core` output-size/profile policy, bounded per-frame pipes, exact CFR/zero-origin preflight, and MP4/H.264/AAC output checks. The native CLI lists wgpu adapter descriptors and accepts/persists an exact descriptor key in an explicit preference file; it re-resolves that key on each run and reports fallback if unavailable. A shared cancellation token now stops either route; scoped child guards kill/reap FFmpeg on error or cancellation, and an unpublished partial output is removed after children exit. The CLI handles Ctrl-C; the GPU map wait checks cancellation between bounded polls. This is **not** zero-copy codec/GPU interoperability: the shared-wgpu route counts one CPU→GPU upload and one GPU→CPU readback per frame, with a completion wait per frame. No Dioxus Native/Blitz UI or live adapter switching has been added. The tested NVIDIA GPU route is slower than direct FFmpeg on the small fixture; this is a measured baseline, not a universal GPU-performance conclusion. See `docs/interop-report.md`. M4's broader fixture/metadata matrix, blocked-pipe/driver-loss recovery, device generation-swapping, hardware surface interop, and full acceptance remain pending.
