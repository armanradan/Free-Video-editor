...
Implementation status (updated 2026-09-25): an explicitly selected `?backend=ffmpeg-wasm` MP4/H.264/AAC compatibility spike shares the M3 browser job, WebCodecs decode, metadata checks, and wgpu processing with the default WebCodecs path. A browser-local `OutputStorageKind` policy now gives both codec output adapters the same OPFS output contract; WebCodecs retains its bounded `StreamTarget` and memory compatibility fallback, whereas FFmpeg requires OPFS. FFmpeg receives one wgpu-produced `VideoFrame.copyTo` RGBA readback at a time, spools it to OPFS, reads the original `File` through Emscripten WORKERFS, and reads/writes the raw spool and final MP4 through worker-side synchronous OPFS file devices. Its prior 64 MiB whole-source and 128 MiB in-memory raw caps are replaced by a 2 GiB raw-disk-spool policy with a browser quota preflight; source size is no longer capped by this adapter. It accepts integer constant rates 1–120 fps plus 24000/1001, 30000/1001, and 60000/1001, and still requires exactly one audio track and exact per-frame timing. The UI marks WebM/HEVC choices unavailable in FFmpeg mode, and WebCodecs remains the default. Both available browsers passed the 60-frame fixture and a valid 257.3 MiB sparse MP4 source through FFmpeg, with full re-decode, independent ffprobe, cancellation, and same-page retry. This is a disk-backed compatibility route, not a no-readback fast path or proof of bounded FFmpeg internal/WASM memory. Core assets are large and GPL-licensed; full browser-process peak resident memory, actual long-duration conversion, broad geometry/VFR cases, distribution compliance, and production resource suitability remain unresolved. See `docs/interop-report.md` for measured results.

Unavailable FFmpeg profiles keep selected input metadata and show the exact rejected reason beside the disabled selector, including a route back to WebCodecs. The 2 GiB raw-RGBA spool bound is an application disk policy, not a source-size cap or a guarantee that all accepted clips fit in FFmpeg's WASM address space. The source and output are streamed at the filesystem boundary, but raw frames are spooled before encoding rather than encoded concurrently with GPU processing. A much larger clip may exhaust OPFS quota or FFmpeg internal memory; neither is silently retried in memory.
...
#### M3.8 — proposed bounded FFmpeg frame stream (not implemented)

**Goal and scope.** Remove the M3.7 *raw intermediate frame* OPFS spool and its 2 GiB policy limit. Do not change the default WebCodecs backend, the shared wgpu processor, codec/profile selection, or the compressed-input/output storage policy. The selected source `File` may still be read through WORKERFS and the final compressed MP4 may still stream to OPFS; “no raw disk persistence” does not mean keeping a whole output movie in memory. FFmpeg WASM still requires one explicit `VideoFrame.copyTo` GPU→CPU RGBA readback per processed frame, followed by a copy into its WASM heap. This milestone cannot claim a no-readback or zero-copy FFmpeg path. It is an optional follow-up to M3.7, not a prerequisite for M4, and the following is a design proposal rather than verified browser behavior.
...
**Protocol and boundedness.** Use a fixed byte budget (initial candidate: four 1 MiB slots), with per-slot atomic `FREE`/`READY` state and published byte length, plus atomic `RUNNING`/`EOF`/`ABORT` control and a job generation. Copy only into a free slot; publish length and then `READY` atomically. The FS `read` callback consumes bytes in strict slot order, supports arbitrary request sizes across slot/frame boundaries, releases each fully consumed slot, and returns zero *only* after EOF and all published bytes have drained. Temporary ring emptiness is not EOF: wait for enough data to satisfy a read, except for the final short read at true EOF. It must not seek backwards; verify that FFmpeg's rawvideo demuxer accepts this non-seekable device before removing the spool. Check multiplication and counters for overflow, including recordings that pass 2 GiB of total bytes. Retain at most one full RGBA frame buffer on the producer side, chunk it into the ring, then reuse it only after its bytes are consumed; the bridge's explicit JS storage bound is one frame plus the ring, not the number of frames. This does **not** bound FFmpeg's opaque WASM heap, browser copies, or the decoded/GPU pools. Keep the current exact constant-frame-rate and timestamp checks until FFmpeg is given a validated way to consume per-frame timing; a byte stream alone carries no VFR timestamps.
...
Core M3 browser robustness is complete when M3.1 through M3.6 pass. M3.7 and proposed M3.8 are optional compatibility extensions and are not required to begin M4. Partial completion must be reported by sub-milestone number and must not be presented as completion of all browser robustness work.
