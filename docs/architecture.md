...
Implementation status (2026-09-25): an explicitly selected `?backend=ffmpeg-wasm` MP4/H.264/AAC compatibility spike now shares the M3 browser job, WebCodecs decode, metadata checks, and wgpu processing with the default WebCodecs path. Only the output adapter changes. Its local single-thread FFmpeg core runs in a nested worker, receives bounded RGBA readbacks from the wgpu-produced `VideoFrame`s, and encodes H.264/AAC with exact MP4 microsecond timescales. It retains a 64 MiB source cap, 128 MiB raw-frame cap, integer constant-frame-rate requirement, and one-audio-track rule. The UI marks WebM/HEVC choices unavailable in this mode, and normal WebCodecs remains the default. Chromium passed the same 60-frame MP4 profile through both backends; Firefox, where WebCodecs MP4/AAC output was unavailable for that fixture, passed the FFmpeg mode. Full browser re-decode, separate ffprobe checks, cancellation, and same-page retry passed. This is a deliberately constrained compatibility spike, not a general FFmpeg transcoder or a no-readback fast path. Core assets are large and GPL-licensed; full browser-process peak resident memory, broad geometry/VFR cases, distribution compliance, and production resource suitability remain unresolved. See `docs/interop-report.md` for measured numbers and the provisional acceptance assessment.

Unavailable FFmpeg profiles must keep the selected input metadata and show the exact rejected bound beside the disabled selector, including a route back to the default WebCodecs backend. The 64 MiB source cap is a conservative application policy because this adapter copies the whole source into the WASM virtual filesystem; it is not a browser or FFmpeg limit. The separate 128 MiB raw-RGBA cap protects the whole-frame accumulation before encoding. Raising either number without a streaming FFmpeg ingress design would not make long recordings safe.
