...
Current M4 gaps: source rotation/non-square-pixel and broader color policy, negative origins and unusual edit lists, VFR support through the GPU route (which needs a timestamp-carrying encoder ingress), genuine device-loss/stuck-GPU recovery, longer high-resolution memory/throughput measurements, and other operating systems/GPUs. Native UI and hardware codec-surface sharing remain M5.
