...
Read `docs/architecture.md` before implementation. M1 established the browser decode → wgpu resize → encode round trip; M3.1 through M3.6 now pass their acceptance gates in the available Firefox/Chromium environments as documented in `docs/interop-report.md`. M3.7 is an optional FFmpeg WASM spike, not an automatic next task. Implement only the milestone requested by the user. Future roadmap phases are not instructions to build everything now.
