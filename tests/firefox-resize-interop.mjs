...
try {
  ...
  await delay(250);
  evidence.beforeInput = JSON.parse(await evaluate('JSON.stringify({profileDisabled:document.querySelector("#output-profile").disabled,convertDisabled:document.querySelector("#convert").disabled,note:Array.from(document.querySelectorAll(".note")).map(node=>node.textContent).find(text=>text.startsWith("Select an input to check output profiles"))})'));
  ...
  await waitFor(`${status}.includes("Output resolves to 640×360") && document.querySelector("#resolved-size")?.textContent.includes("640×360")`);
  assert.equal(await evaluate('document.querySelector("#resize-preset").value'), 'original');
  evidence.afterInput = JSON.parse(await evaluate('JSON.stringify({profileDisabled:document.querySelector("#output-profile").disabled,convertDisabled:document.querySelector("#convert").disabled,mp4Disabled:document.querySelector("#output-profile option[value=\\"mp4-h264-aac\\"]").disabled})'));
  assert.equal(evidence.afterInput.profileDisabled, false);
  assert.equal(evidence.afterInput.convertDisabled, false);
  assert.equal(evidence.afterInput.mp4Disabled, true);
